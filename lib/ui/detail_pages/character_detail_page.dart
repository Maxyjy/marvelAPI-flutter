...
class CharacterDetailPageTest extends StatefulWidget {
  final TestDataResult character;
  CharacterDetailPageTest({Key key, @required this.character}) : super(key: key);

  @override
  _TabPageState createState() => new _TabPageState(character);
}

class _TabPageState extends State<CharacterDetailPageTest>
    with AutomaticKeepAliveClientMixin {
  _TabPageState(TestDataResult character) {
    this.character = character;
  }
  TestDataResult character;
  ...
  String _getDetailEventCharacterUrl = "/v1/public/characters/";
  ...
  Future<bool> getDetailComic() async {
    ...
    String url =
        _host + _getDetailEventCharacterUrl + character.id.toString() + '/comics';
    print(url);
    Dio dio = Dio();
    final response = await dio.get(
      url,
  ...
  Future<bool> getDetailEvents() async {
    ...
    String url =
        _host + _getDetailEventCharacterUrl + character.id.toString() + '/events';
    print(url);
    Dio dio = Dio();
    final response = await dio.get(
      url,
  ...
  Future<bool> getDetailSeries() async {
    ...
    String url =
        _host + _getDetailEventCharacterUrl + character.id.toString() + '/series';
    print(url);
    Dio dio = Dio();
    final response = await dio.get(
      url,
  ...
  firstEventsRefresh() async {
    ...
    await Future.delayed(Duration(seconds: 0), () {
      setState(() {
        ...
        _eventscount=eventsList.length;
...
//                                ),
          ...
          Container(
            ...
            child: EasyRefresh.custom(
              ...
              onLoad: () async {
                ...
                await Future.delayed(Duration(seconds: 0), () {
                  setState(() {
                    _count += detailEventSeriesEntity.data.count;
                  });
                });
              },
            ),
  ...
  Widget buildStories() {
    ...
    String temp='';
    for(int i=0;i<=character.stories.returned-1;i++)
    {
      if(character.stories.items[i].name.length==0){
        temp="( Name Not Found )"+'\n';

      }else {
        temp = temp + character.stories.items[i].name + '\n';
      }
    }
    if(character.stories.returned==20) {
      temp = temp + '...' +
          (character.stories.available - character.stories.returned).toString() +
          ' Stories';
    }
    return  Container(
      margin: EdgeInsets.only(top: 15),
      ...
      Container(
          padding: EdgeInsets.fromLTRB(15, 0, 0, 0),
          alignment: Alignment.centerLeft,child: Text('Available: '+character.stories.available.toString(),textScaleFactor: 1.0,style: TextStyle(fontSize: 18,height: 1.2,color: MyColors.detail_text_color),)
      ),
      Container(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Character',textScaleFactor: 1.0,),
//        Row(
//          children: <Widget>[
//            Expanded(
//                child: Container(
